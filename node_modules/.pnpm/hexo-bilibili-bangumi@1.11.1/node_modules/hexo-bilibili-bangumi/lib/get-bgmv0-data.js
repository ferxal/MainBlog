...
var getData = /*#__PURE__*/function () {
  var _ref2 = (0, _asyncToGenerator2["default"])(/*#__PURE__*/_regenerator["default"].mark(function _callee2(vmid, status, typeNum, pn, coverMirror) {
    var response, $data, _response$data2, list, _iterator, _step, _bangumi$subject, _bangumi$subject2, _bangumi$subject3, _bangumi$subject4, _bangumi$subject5, _bangumi$subject$scor, _bangumi$subject6, _bangumi$subject7, _bangumi$subject8, _bangumi$subject9, _bangumi$subject0, bangumi;
...
var processData = /*#__PURE__*/function () {
      ...
      while (1) switch (_context3.prev = _context3.next) {
        ...
        case 2:
          ...
          if (showProgress) {
            ...
            bar = new ProgressBar("\u6B63\u5728\u83B7\u53D6 ".concat(TYPE_STATUS[typeNum][status - 1], " ").concat(TYPE[typeNum], " [:bar] :percent :elapseds"), {
              total: page.data - 1,
              complete: '█'
            });
          }
        ...
        case 18:
          console.log("Get ".concat(TYPE_EN[typeNum], " data error:"), page === null || page === void 0 ? void 0 : page.data);
  ...
  return function processData(_x9, _x0, _x1, _x10, _x11) {
    ...
    return _regenerator["default"].wrap(function _callee4$(_context4) {
      while (1) switch (_context4.prev = _context4.next) {
        case 0:
          ...
          log.info("Getting bgm ".concat(TYPE_EN[type], " data, please wait..."));
        ...
        case 11:
          ...
          log.info("".concat(wantWatch.length + watching.length + watched.length, " ").concat(TYPE_EN[type], "s have been loaded in ").concat(endTime - startTime, " ms"));
          bangumis = {
            wantWatch: wantWatch,
            watching: watching,
            watched: watched
          ...
          fs.writeFile(path.join(sourceDir, "/_data/".concat(TYPE_EN[type], "s.json")), JSON.stringify(bangumis), function (err) {
            if (err) {
              log.info("Failed to write data to _data/".concat(TYPE_EN[type], "s.json"));
              console.error(err);
            } else {
              log.info("Bgm ".concat(TYPE_EN[type], "s data has been saved"));
            }
          });
          if (pagination) {
            allBangumis = _objectSpread({}, bangumis); // extra bangumis
            if (fs.existsSync(path.join(sourceDir, "/_data/extra_".concat(TYPE_EN[type], "s.json")))) {
              _JSON$parse = JSON.parse(fs.readFileSync(path.join(_this.source_dir, "/_data/extra_".concat(TYPE_EN[type], "s.json")))), wantWatchExtra = _JSON$parse.wantWatchExtra, watchingExtra = _JSON$parse.watchingExtra, watchedExtra = _JSON$parse.watchedExtra;
              if (wantWatchExtra) {
                if (extraOrder === 1) {
            ...
            fs.writeFile(path.join(sourceDir, "/".concat(TYPE_EN[type], "s.json")), JSON.stringify(bangumis), function (err) {
              if (err) {
                log.info("Failed to write data to ".concat(TYPE_EN[type], "s.json"));
                console.error(err);
              } else {
                log.info("Bgm ".concat(TYPE_EN[type], "s data has been saved"));
              }
